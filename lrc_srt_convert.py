...
def __convert_file(data, path):
    if path.split(".")[len(path.split("."))-1] == "lrc":
        ...
        for a in range(len(mm_list)):
            file.write('['+mm_list[a]+':'+ss_list[a] +
                       '.'+mms_list[a]+']'+content_list[a])
        file.close()
    ...
    if path.split(".")[len(path.split("."))-1] == "srt":
        ...
        for a in range(len(hh_list)-1):
            ...
            file.write(str(hh_list[a])+':'+str(mm_list[a])+':'+str(ss_list[a])+','
                       + str(mms_list[a])+" --> " +
                       str(hh_list[a+1])+':'+str(mm_list[a+1])
...
def convert(src, dst):
    data = __load_file(src)
    __convert_file(data, dst)
